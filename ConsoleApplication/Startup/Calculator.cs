...
namespace Startup
{
    public class Calculator 
    {
        ...
        private static int CalculateSubtractionExpression(string subtractionExpression)
        {
            ...
            var result = CalculateMultiplicationExpression(parts[0]);

            for (var i = 1; i < parts.Count(); i++)
            {
                result -= CalculateMultiplicationExpression(parts[i]);
            }

            return result;
        }

        private static int CalculateMultiplicationExpression(string multiplicationExpression)
        {
            var parts = multiplicationExpression.Split(MultiplicationSymbol);
